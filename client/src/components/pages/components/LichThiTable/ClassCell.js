...
const getMonChonRoiKey = data => `${data.MaMH}-${data.ThucHanh}`
const useMonChonRoi = () => {
...
export const [ClassCellContext, useClassCellContext] = constate(() => {
    ...
    const isHoveringOnThisCell = (data, fieldCompare) => {
        return cellHovering?.[fieldCompare] === data?.[fieldCompare]
    }
    const isHoveringOnThisCellRemoveIcon = data =>
        isHoveringOnThisCell(data, "MaMH") && isHoveringOnRemoveIcon
    const isHoveringOnThisCellWarningIcon = data => {
        return (
            !!cellHovering &&
                getMonChonRoiKey(data) === getMonChonRoiKey(cellHovering) &&
