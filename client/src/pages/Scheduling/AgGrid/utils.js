...
const columnDefs = [
  ...
  },
  {
    ...
    valueGetter: ({ data }) => {
      ...
      //return `Thứ ${parseInt(data.Thu)} ${BUOI_FORMAT_MAP[buoi]}`
      return `Thứ ${parseInt(data.Thu)}`
    },
    // comparator: (a, b) => {
...
export const useGridOptions = () => {
    ...
    const updateNodesSelectionToAgGrid = useCallback(selectedClasses => {
        ...
        api.forEachNode(node => {
            if (
                //selectedClasses.find(it => node.data && isSameAgGridRowId(it, node.data))
                selectedClasses.find(it => node.data)
            ) {
        ...
        if (toSelectNodes.length) {
            api.setNodesSelected({
                nodes: toSelectNodes,
                newValue: true,
                source: PROGRAMMATICALLY_CHANGE_SELECTION
            })
        }
    }, [])
    ...
    // const onFilterChanged = useDebouncedCallback(e => {
    ...
    // const onColumnChanged = useDebouncedCallback(({ columnApi }) => {
    //     //log(">>onColumnChanged")
    ...
    const onRowClicked = useCallback(({ node }) => {
        ...
        if (node.data && !node.selectable) {
            ...
            // enqueueSnackbar(
            //     `Không thể chọn lớp ${node.data.MaLop} do bị trùng TKB với lớp đã chọn`,
            //     {
            //         variant: "warning",
    ...
    const getContextMenuItems = useCallback(({ value, column, api, columnApi }) => {
        const {
            addToBlock,
            endOfBlock,
            constructFinal
        } = getContextMenuItemsBuilder()
        ...
        if (value && column?.isFilterAllowed()) {
            ...
            const alreadyFilterByThisValue =
                thisColumnCurrentFilterModel?.filter === value ||
                    thisColumnCurrentFilterModel?.values?.includes(value)
        ...
        if (column?.isFilterAllowed() && api.isColumnFilterPresent()) {
            const {
                [column.getColId()]: thisColumnFilterModel,
                ...otherColumnsFilterModel
            } = api.getFilterModel()
            if (thisColumnFilterModel) {
                addToBlock({
                    name: `Reset Filter For "${headerName}"`,
                    action: () => {
                        // tracker.track("[page2] context_menu_reset_1_filter_clicked", {
                        //     headerName
                        // })
                        api.setFilterModel({
                            ...api.getFilterModel(),
            ...
            if (thisColumnFilterModel && Object.keys(otherColumnsFilterModel).length) {
                addToBlock({
                    ...
                    action: () => {
                        // tracker.track("[page2] context_menu_reset_n_1_filters_clicked", {
                        //     headerName
                        // })
                        api.setFilterModel({
